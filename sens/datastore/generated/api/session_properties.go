...
func UpdateSessionPropertieWhere(w http.ResponseWriter, r *http.Request) {
	...
	if data, err := ioutil.ReadAll(r.Body); err != nil {
		logger.Error(err)
		http.Error(w, err.Error(), http.StatusInternalServerError)
	} else if err := fn.UpdateSessionPropertieWhere(or, and, in, span, data); err != nil {
		logger.Error(err)
		http.Error(w, err.Error(), http.StatusInternalServerError)
	} else {
...
func FindSessionPropertie(w http.ResponseWriter, r *http.Request) {
	...
	} else if ms, err := fn.FindSessionPropertie(or, and, in, span, limit, column, order); err != nil {
		logger.Error(err)
		http.Error(w, err.Error(), http.StatusInternalServerError)
