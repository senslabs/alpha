...
func buildUserSummaryViewWhereClause(query *bytes.Buffer, or []string, and []string, in string, span []string, values map[string]interface{}) {
...
func findUserSummaryViewIn(seq int64, query string, values map[string]interface{}) ([]models.UserSummaryView, *errors.SensError) {
...
func FindUserSummaryView(or []string, and []string, in string, span []string, limit string, column string, order string) ([]models.UserSummaryView, *errors.SensError) {
	from := time.Now().Unix()
	seq := from % 10000
	...
	if strings.TrimSpace(in) == "" {
		...
		m, err := findUserSummaryViewIn(seq, q, values)
...
func UpdateUserSummaryViewWhere(or []string, and []string, in string, span []string, data []byte) *errors.SensError {
	fieldMap := models.GetUserSummaryViewFieldMap()
	values := make(map[string]interface{})
	...
	for k, _ := range j {
		if f, ok := fieldMap[k]; ok {
			...
			values["set_"+f] = getUserSummaryViewFieldValue(k, j[k])
			comma = ", "
		}
	}
	//SET ENDS
