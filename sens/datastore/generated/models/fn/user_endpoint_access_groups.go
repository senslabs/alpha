...
func BatchInsertUserEndpointAccessGroup(data []byte) {
	...
	fmt.Fprint(insert, " ON CONFLICT(user_id,endpoint_category) DO UPDATE SET (", strings.Join(fields, ", "), ") = (EXCLUDED.", strings.Join(fields, ", EXCLUDED."), ")")
...
func BatchUpsertUserEndpointAccessGroup(data []byte) {
	var j []map[string]interface{}
	types.Unmarshal(data, &j)
	if len(j) == 0 {
		return
...
func buildUserEndpointAccessGroupWhereClause(query *bytes.Buffer, or []string, and []string, in string, span []string, values* []interface{}) {
...
func FindUserEndpointAccessGroup(or []string, and []string, in string, span []string, limit string, column string, order string) []map[string]interface{} {
	...
	result := datastore.RowsToMap(r, models.GetUserEndpointAccessGroupReverseFieldMap(), models.GetUserEndpointAccessGroupTypeMap())
	return result
}

func UpdateUserEndpointAccessGroupWhere(or []string, and []string, in string, span []string, data []byte) {
	var values []interface{}
	j := types.UnmarshalMap(data)
	fieldMap := models.GetUserEndpointAccessGroupFieldMap()
	typeMap := models.GetUserEndpointAccessGroupTypeMap()
...
func DeleteUserEndpointAccessGroup(or []string, and []string, in string, span []string) int64 {
	query := bytes.NewBufferString("DELETE FROM user_endpoint_access_groups WHERE ")
	var values []interface{}
	buildUserEndpointAccessGroupWhereClause(query, or, and, in, span, &values)
	q := query.String()
	logger.Debug(q)
