...
func BatchInsertAlert(data []byte) ([]string, error) {
	var j []map[string]interface{}
	if err := json.Unmarshal(data, &j); err != nil {
		logger.Error(err)
...
func UpdateAlert(id string, data []byte) error {
	var j map[string]interface{}
	if err := json.Unmarshal(data, &j); err != nil {
		logger.Error(err)
		return errors.FromError(errors.GO_ERROR, err)
	...
	fmt.Fprint(update, " WHERE alert_id = :alert_id")
...
func SelectAlert(id string) (models.Alert, *errors.SensError) {
	db := datastore.GetConnection()
	m := models.Alert{}
	if err := db.Get(&m, "SELECT * FROM alerts WHERE alert_id = $1", id); err != nil {
		logger.Error(err)
...
func buildAlertWhereClause(query *bytes.Buffer, or []string, and []string, in string, span []string, values map[string]interface{}) {
	...
	for _, o := range ors {
		if f, ok := fieldMap[o.Column]; ok {
			...
			values[f] = getAlertFieldValue(o.Column, o.Value)
			cond = "OR "
		}
	}
	...
	for _, s := range spans {
		if f, ok := fieldMap[s.Column]; ok {
			...
			values["from_"+f] = getAlertFieldValue(s.Column, s.From)
			values["to_"+f] = getAlertFieldValue(s.Column, s.To)
		}
	}
...
func findAlertIn(seq int64, query string, values map[string]interface{}) ([]models.Alert, *errors.SensError) {
...
func FindAlert(or []string, and []string, in string, span []string, limit string, column string, order string) ([]models.Alert, *errors.SensError) {
	...
	if strings.TrimSpace(in) == "" {
		...
		m, err := findAlertIn(seq, q, values)
...
func UpdateAlertWhere(or []string, and []string, in string, span []string, data []byte) *errors.SensError {
	...
	for k, _ := range j {
		if f, ok := fieldMap[k]; ok {
			...
			values["set_"+f] = getAlertFieldValue(k, j[k])
			comma = ", "
		}
	}
	//SET ENDS
