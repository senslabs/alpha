...
func BatchUpsertOrgActivityView(data []byte) {
	var j []map[string]interface{}
	types.Unmarshal(data, &j)
	if len(j) == 0 {
		return
	...
	fieldMap := models.GetOrgActivityViewFieldMap()
	typeMap := models.GetOrgActivityViewTypeMap()
	insert := bytes.NewBufferString("UPSERT INTO org_activity_views(")
	for k, _ := range j[0] {
		if f, ok := fieldMap[k]; ok {
			fmt.Fprint(insert, comma, f)
			keys = append(keys, k)
...
func DeleteOrgActivityView(or []string, and []string, in string, span []string) int64 {
	query := bytes.NewBufferString("DELETE FROM org_activity_views WHERE ")
	var values []interface{}
	buildOrgActivityViewWhereClause(query, or, and, in, span, &values)
	q := query.String()
	logger.Debug(q)
