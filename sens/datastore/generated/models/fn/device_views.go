...
func BatchUpsertDeviceView(data []byte) {
	var j []map[string]interface{}
	types.Unmarshal(data, &j)
	if len(j) == 0 {
		return
...
func DeleteDeviceView(or []string, and []string, in string, span []string) int64 {
	query := bytes.NewBufferString("DELETE FROM device_views WHERE ")
	var values []interface{}
	buildDeviceViewWhereClause(query, or, and, in, span, &values)
	q := query.String()
	logger.Debug(q)
