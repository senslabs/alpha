...
func buildSessionRecordWhereClause(query *bytes.Buffer, or []string, and []string, in string, span []string, values map[string]interface{}) {
...
func findSessionRecordIn(seq int64, query string, values map[string]interface{}) ([]models.SessionRecord, *errors.SensError) {
...
func FindSessionRecord(or []string, and []string, in string, span []string, limit string, column string, order string) ([]models.SessionRecord, *errors.SensError) {
	from := time.Now().Unix()
	seq := from % 10000
	...
	if strings.TrimSpace(in) == "" {
		...
		m, err := findSessionRecordIn(seq, q, values)
...
func UpdateSessionRecordWhere(or []string, and []string, in string, span []string, data []byte) *errors.SensError {
	fieldMap := models.GetSessionRecordFieldMap()
	values := make(map[string]interface{})
