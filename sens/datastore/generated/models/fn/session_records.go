...
func InsertSessionRecord(data []byte) string {
...
func BatchInsertSessionRecord(data []byte) {
	...
	fmt.Fprint(insert, " ON CONFLICT(user_id,key,timestamp) DO UPDATE SET (", strings.Join(fields, ", "), ") = (EXCLUDED.", strings.Join(fields, ", EXCLUDED."), ")")
...
func BatchUpsertSessionRecord(data []byte) {
	var j []map[string]interface{}
	types.Unmarshal(data, &j)
	if len(j) == 0 {
		return
...
func buildSessionRecordWhereClause(query *bytes.Buffer, or []string, and []string, in string, span []string, values* []interface{}) {
...
func FindSessionRecord(or []string, and []string, in string, span []string, limit string, column string, order string) []map[string]interface{} {
	query := bytes.NewBufferString("SELECT * FROM session_records WHERE ")
	fieldMap := models.GetSessionRecordFieldMap()
	var values []interface{}
	buildSessionRecordWhereClause(query, or, and, in, span, &values)
	if column == "" {
		column = "created_at"
	}
	...
	result := datastore.RowsToMap(r, models.GetSessionRecordReverseFieldMap(), models.GetSessionRecordTypeMap())
	return result
}

func UpdateSessionRecordWhere(or []string, and []string, in string, span []string, data []byte) {
	var values []interface{}
	j := types.UnmarshalMap(data)
	fieldMap := models.GetSessionRecordFieldMap()
	typeMap := models.GetSessionRecordTypeMap()
	...
	buildSessionRecordWhereClause(update, or, and, in, span, &values)
...
func DeleteSessionRecord(or []string, and []string, in string, span []string) int64 {
	query := bytes.NewBufferString("DELETE FROM session_records WHERE ")
	var values []interface{}
	buildSessionRecordWhereClause(query, or, and, in, span, &values)
	q := query.String()
	logger.Debug(q)
