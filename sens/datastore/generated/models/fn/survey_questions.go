...
func BatchUpsertSurveyQuestion(data []byte) {
	var j []map[string]interface{}
	types.Unmarshal(data, &j)
	if len(j) == 0 {
		return
...
func UpdateSurveyQuestion(id string, data []byte) {
	...
	fmt.Fprint(update, " WHERE survey_question_id = $", phi)
...
func SelectSurveyQuestion(id string) map[string]interface{} {
	...
	stmt, err := db.Prepare("SELECT * FROM survey_questions WHERE survey_question_id = $1")
	defer stmt.Close()
	errors.Pie(err)
...
func DeleteSurveyQuestion(or []string, and []string, in string, span []string) int64 {
	query := bytes.NewBufferString("DELETE FROM survey_questions WHERE ")
	var values []interface{}
	buildSurveyQuestionWhereClause(query, or, and, in, span, &values)
	q := query.String()
	logger.Debug(q)
