...
func SelectOrgAuth(id string) models.OrgAuth {
	db := models.GetConnection()
	m := models.OrgAuth{}
	err := db.Get(&m, "SELECT * FROM org_auths WHERE id = $1", id)
	if err != nil {
		log.Println(err)
	}
	return m
...
func FindOrgAuth(or []string, and []string, span []string) []models.OrgAuth {
