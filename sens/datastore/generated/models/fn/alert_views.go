...
func InsertAlertView(data []byte) string {
	...
	fieldMap := models.GetAlertViewFieldMap()
	typeMap := models.GetAlertViewTypeMap()
	insert := bytes.NewBufferString("INSERT INTO alert_views(")
...
func BatchInsertAlertView(data []byte) {
...
func buildAlertViewWhereClause(query *bytes.Buffer, or []string, and []string, in string, span []string, values* []interface{}) {
...
func FindAlertView(or []string, and []string, in string, span []string, limit string, column string, order string) []map[string]interface{} {
	query := bytes.NewBufferString("SELECT * FROM alert_views WHERE ")
	fieldMap := models.GetAlertViewFieldMap()
	var values []interface{}
	buildAlertViewWhereClause(query, or, and, in, span, &values)
	if column == "" {
		column = "created_at"
	}
	...
	result := datastore.RowsToMap(r, models.GetAlertViewReverseFieldMap(), models.GetAlertViewTypeMap())
	return result
}

func UpdateAlertViewWhere(or []string, and []string, in string, span []string, data []byte) {
	var values []interface{}
	j := types.UnmarshalMap(data)
	fieldMap := models.GetAlertViewFieldMap()
	typeMap := models.GetAlertViewTypeMap()
	update := bytes.NewBufferString("UPDATE alert_views SET ")
	...
	buildAlertViewWhereClause(update, or, and, in, span, &values)
...
func DeleteAlertView(or []string, and []string, in string, span []string) int64 {
	query := bytes.NewBufferString("DELETE FROM alert_views WHERE ")
	var values []interface{}
	buildAlertViewWhereClause(query, or, and, in, span, &values)
	q := query.String()
	logger.Debug(q)
