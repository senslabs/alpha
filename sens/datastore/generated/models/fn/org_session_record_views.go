...
func BatchUpsertOrgSessionRecordView(data []byte) {
	var j []map[string]interface{}
	types.Unmarshal(data, &j)
	if len(j) == 0 {
		return
	...
	fieldMap := models.GetOrgSessionRecordViewFieldMap()
	typeMap := models.GetOrgSessionRecordViewTypeMap()
	insert := bytes.NewBufferString("UPSERT INTO org_session_record_views(")
	for k, _ := range j[0] {
		if f, ok := fieldMap[k]; ok {
			fmt.Fprint(insert, comma, f)
			keys = append(keys, k)
...
func DeleteOrgSessionRecordView(or []string, and []string, in string, span []string) int64 {
	query := bytes.NewBufferString("DELETE FROM org_session_record_views WHERE ")
	var values []interface{}
	buildOrgSessionRecordViewWhereClause(query, or, and, in, span, &values)
	q := query.String()
	logger.Debug(q)
