...
func buildUserEndpointWhereClause(query *bytes.Buffer, or []string, and []string, in string, span []string, values map[string]interface{}) {
	...
	for _, o := range ors {
		if f, ok := fieldMap[o.Column]; ok {
			...
			values[f] = getUserEndpointFieldValue(o.Column, o.Value)
			cond = "OR "
		}
	}
	...
	for _, a := range ands {
		if f, ok := fieldMap[a.Column]; ok {
			...
			values[f] = getUserEndpointFieldValue(a.Column, a.Value)
		}
	}
	...
	for _, s := range spans {
		if f, ok := fieldMap[s.Column]; ok {
			...
			values["from_"+f] = getUserEndpointFieldValue(s.Column, s.From)
			values["to_"+f] = getUserEndpointFieldValue(s.Column, s.To)
		}
	}
	fmt.Fprint(query, "1 = 1)")
}

func getUserEndpointFieldValue(c string, v interface{}) interface{} {
...
func findUserEndpointIn(seq int64, query string, values map[string]interface{}) ([]models.UserEndpoint, *errors.SensError) {
...
func FindUserEndpoint(or []string, and []string, in string, span []string, limit string, column string, order string) ([]models.UserEndpoint, *errors.SensError) {
	from := time.Now().Unix()
	seq := from % 10000
	...
	if strings.TrimSpace(in) == "" {
		...
		m, err := findUserEndpointIn(seq, q, values)
...
func UpdateUserEndpointWhere(or []string, and []string, in string, span []string, data []byte) *errors.SensError {
	fieldMap := models.GetUserEndpointFieldMap()
	values := make(map[string]interface{})
	...
	for k, _ := range j {
		if f, ok := fieldMap[k]; ok {
			...
			values["set_"+f] = getUserEndpointFieldValue(k, j[k])
			comma = ", "
		}
	}
	//SET ENDS
