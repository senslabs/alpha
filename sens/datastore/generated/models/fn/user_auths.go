...
func SelectUserAuth(id string) models.UserAuth {
	db := models.GetConnection()
	m := models.UserAuth{}
	err := db.Get(&m, "SELECT * FROM user_auths WHERE id = $1", id)
	if err != nil {
		log.Println(err)
	}
	return m
...
func FindUserAuth(or []string, and []string, span []string) []models.UserAuth {
