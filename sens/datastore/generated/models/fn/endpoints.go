...
func BatchInsertEndpoint(data []byte) ([]string, error) {
	...
	var keys []string
	fieldMap := models.GetEndpointFieldMap()
	insert := bytes.NewBufferString("UPSERT INTO endpoints(")
...
func UpdateEndpoint(id string, data []byte) error {
	...
	fmt.Fprint(update, " WHERE endpoint_id = :endpoint_id")
...
func SelectEndpoint(id string) (models.Endpoint, *errors.SensError) {
	db := datastore.GetConnection()
	m := models.Endpoint{}
	if err := db.Get(&m, "SELECT * FROM endpoints WHERE endpoint_id = $1", id); err != nil {
		logger.Error(err)
...
func buildEndpointWhereClause(query *bytes.Buffer, or []string, and []string, in string, span []string, values map[string]interface{}) {
...
func findEndpointIn(seq int64, query string, values map[string]interface{}) ([]models.Endpoint, *errors.SensError) {
...
func FindEndpoint(or []string, and []string, in string, span []string, limit string, column string, order string) ([]models.Endpoint, *errors.SensError) {
	...
	if strings.TrimSpace(in) == "" {
		...
		m, err := findEndpointIn(seq, q, values)
...
func UpdateEndpointWhere(or []string, and []string, in string, span []string, data []byte) *errors.SensError {
	fieldMap := models.GetEndpointFieldMap()
	values := make(map[string]interface{})
	update := bytes.NewBufferString("UPDATE endpoints SET ")
	...
	for k, _ := range j {
		if f, ok := fieldMap[k]; ok {
			...
			values["set_"+f] = getEndpointFieldValue(k, j[k])
			comma = ", "
		}
	}
	//SET ENDS
