...
func InsertRecorderView(data []byte) string {
	...
	fieldMap := models.GetRecorderViewFieldMap()
	typeMap := models.GetRecorderViewTypeMap()
	insert := bytes.NewBufferString("INSERT INTO recorder_views(")
...
func BatchUpsertRecorderView(data []byte) {
	var j []map[string]interface{}
	types.Unmarshal(data, &j)
	if len(j) == 0 {
		return
	...
	fieldMap := models.GetRecorderViewFieldMap()
	typeMap := models.GetRecorderViewTypeMap()
	insert := bytes.NewBufferString("UPSERT INTO recorder_views(")
	for k, _ := range j[0] {
		if f, ok := fieldMap[k]; ok {
			fmt.Fprint(insert, comma, f)
			keys = append(keys, k)
...
func buildRecorderViewWhereClause(query *bytes.Buffer, or []string, and []string, in string, span []string, values* []interface{}) {
...
func FindRecorderView(or []string, and []string, in string, span []string, limit string, column string, order string) []map[string]interface{} {
	query := bytes.NewBufferString("SELECT * FROM recorder_views WHERE ")
	fieldMap := models.GetRecorderViewFieldMap()
	var values []interface{}
	buildRecorderViewWhereClause(query, or, and, in, span, &values)
	if column == "" {
		column = "created_at"
	}
	...
	result := datastore.RowsToMap(r, models.GetRecorderViewReverseFieldMap(), models.GetRecorderViewTypeMap())
	return result
}

func UpdateRecorderViewWhere(or []string, and []string, in string, span []string, data []byte) {
	var values []interface{}
	j := types.UnmarshalMap(data)
	fieldMap := models.GetRecorderViewFieldMap()
	typeMap := models.GetRecorderViewTypeMap()
	update := bytes.NewBufferString("UPDATE recorder_views SET ")
	...
	buildRecorderViewWhereClause(update, or, and, in, span, &values)
...
func DeleteRecorderView(or []string, and []string, in string, span []string) int64 {
	query := bytes.NewBufferString("DELETE FROM recorder_views WHERE ")
	var values []interface{}
	buildRecorderViewWhereClause(query, or, and, in, span, &values)
	q := query.String()
	logger.Debug(q)
