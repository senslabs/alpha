...
func SelectOpUser(id string) models.OpUser {
	db := models.GetConnection()
	m := models.OpUser{}
	err := db.Get(&m, "SELECT * FROM op_users WHERE id = $1", id)
	if err != nil {
		log.Println(err)
	}
	return m
...
func FindOpUser(or []string, and []string, span []string) []models.OpUser {
