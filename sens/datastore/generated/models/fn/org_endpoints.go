...
func buildOrgEndpointWhereClause(query *bytes.Buffer, or []string, and []string, span []string, values map[string]interface{}) {
...
func FindOrgEndpoint(or []string, and []string, span []string, limit string, column string, order string) ([]models.OrgEndpoint, *errors.SensError) {
	query := bytes.NewBufferString("SELECT * FROM org_endpoints WHERE ")
	fieldMap := models.GetOrgEndpointFieldMap()
	values := make(map[string]interface{})
	buildOrgEndpointWhereClause(query, or, and, span, values)
	if column != "" {
...
func UpdateOrgEndpointWhere(or []string, and []string, span []string, data []byte) *errors.SensError {
	fieldMap := models.GetOrgEndpointFieldMap()
	values := make(map[string]interface{})
	...
	buildOrgEndpointWhereClause(update, or, and, span, values)
