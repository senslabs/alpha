...
func InsertOrgEndpoint(data []byte) (string, error) {
...
func BatchInsertOrgEndpoint(data []byte) ([]string, error) {
	var j []map[string]interface{}
	if err := json.Unmarshal(data, &j); err != nil {
		logger.Error(err)
...
func buildOrgEndpointWhereClause(query *bytes.Buffer, or []string, and []string, in string, span []string, values map[string]interface{}) {
	...
	for _, o := range ors {
		if f, ok := fieldMap[o.Column]; ok {
			...
			values[f] = getOrgEndpointFieldValue(o.Column, o.Value)
			cond = "OR "
		}
	}
	...
	for _, a := range ands {
		if f, ok := fieldMap[a.Column]; ok {
			...
			values[f] = getOrgEndpointFieldValue(a.Column, a.Value)
		}
	}
	...
	for _, s := range spans {
		if f, ok := fieldMap[s.Column]; ok {
			...
			values["from_"+f] = getOrgEndpointFieldValue(s.Column, s.From)
			values["to_"+f] = getOrgEndpointFieldValue(s.Column, s.To)
		}
	}
	fmt.Fprint(query, "1 = 1)")
}

func getOrgEndpointFieldValue(c string, v interface{}) interface{} {
...
func findOrgEndpointIn(seq int64, query string, values map[string]interface{}) ([]models.OrgEndpoint, *errors.SensError) {
...
func FindOrgEndpoint(or []string, and []string, in string, span []string, limit string, column string, order string) ([]models.OrgEndpoint, *errors.SensError) {
	from := time.Now().Unix()
	seq := from % 10000
	...
	if strings.TrimSpace(in) == "" {
		...
		m, err := findOrgEndpointIn(seq, q, values)
...
func UpdateOrgEndpointWhere(or []string, and []string, in string, span []string, data []byte) *errors.SensError {
	fieldMap := models.GetOrgEndpointFieldMap()
	values := make(map[string]interface{})
	...
	for k, _ := range j {
		if f, ok := fieldMap[k]; ok {
			...
			values["set_"+f] = getOrgEndpointFieldValue(k, j[k])
			comma = ", "
		}
	}
	//SET ENDS
