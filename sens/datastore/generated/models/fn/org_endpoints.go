...
func UpdateOrgEndpoint(id string, data []byte) error {
	var j map[string]interface{}
	if err := json.Unmarshal(data, &j); err != nil {
		logger.Error(err)
		return errors.FromError(errors.GO_ERROR, err)
...
func SelectOrgEndpoint(id string) (models.OrgEndpoint, *errors.SensError) {
	db := models.GetConnection()
	m := models.OrgEndpoint{}
	if err := db.Get(&m, "SELECT * FROM org_endpoints WHERE id = $1", id); err != nil {
		logger.Error(err)
