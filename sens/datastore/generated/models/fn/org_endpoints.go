...
func BatchUpsertOrgEndpoint(data []byte) {
	var j []map[string]interface{}
	types.Unmarshal(data, &j)
	if len(j) == 0 {
		return
...
func DeleteOrgEndpoint(or []string, and []string, in string, span []string) int64 {
	query := bytes.NewBufferString("DELETE FROM org_endpoints WHERE ")
	var values []interface{}
	buildOrgEndpointWhereClause(query, or, and, in, span, &values)
	q := query.String()
	logger.Debug(q)
