...
func buildSleepSummarieWhereClause(query *bytes.Buffer, or []string, and []string, in string, span []string, values map[string]interface{}) {
...
func findSleepSummarieIn(seq int64, query string, values map[string]interface{}) ([]models.SleepSummarie, *errors.SensError) {
...
func FindSleepSummarie(or []string, and []string, in string, span []string, limit string, column string, order string) ([]models.SleepSummarie, *errors.SensError) {
	from := time.Now().Unix()
	seq := from % 10000
	...
	if strings.TrimSpace(in) == "" {
		...
		m, err := findSleepSummarieIn(seq, q, values)
...
func UpdateSleepSummarieWhere(or []string, and []string, in string, span []string, data []byte) *errors.SensError {
	fieldMap := models.GetSleepSummarieFieldMap()
	values := make(map[string]interface{})
	...
	for k, _ := range j {
		if f, ok := fieldMap[k]; ok {
			...
			values["set_"+f] = getSleepSummarieFieldValue(k, j[k])
			comma = ", "
		}
	}
	//SET ENDS
