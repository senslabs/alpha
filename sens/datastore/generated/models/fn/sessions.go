...
func UpdateSession(id string, data []byte) error {
	...
	fmt.Fprint(update, " WHERE session_id = :session_id")
...
func SelectSession(id string) (models.Session, *errors.SensError) {
	db := datastore.GetConnection()
	m := models.Session{}
	if err := db.Get(&m, "SELECT * FROM sessions WHERE session_id = $1", id); err != nil {
		logger.Error(err)
...
func buildSessionWhereClause(query *bytes.Buffer, or []string, and []string, in string, span []string, values map[string]interface{}) {
	...
	for _, o := range ors {
		if f, ok := fieldMap[o.Column]; ok {
			...
			values[f] = getSessionFieldValue(o.Column, o.Value)
			cond = "OR "
		}
	}
	...
	for _, s := range spans {
		if f, ok := fieldMap[s.Column]; ok {
			...
			values["from_"+f] = getSessionFieldValue(s.Column, s.From)
			values["to_"+f] = getSessionFieldValue(s.Column, s.To)
		}
	}
...
func findSessionIn(seq int64, query string, values map[string]interface{}) ([]models.Session, *errors.SensError) {
...
func FindSession(or []string, and []string, in string, span []string, limit string, column string, order string) ([]models.Session, *errors.SensError) {
	...
	if strings.TrimSpace(in) == "" {
		...
		m, err := findSessionIn(seq, q, values)
...
func UpdateSessionWhere(or []string, and []string, in string, span []string, data []byte) *errors.SensError {
	...
	for k, _ := range j {
		if f, ok := fieldMap[k]; ok {
			...
			values["set_"+f] = getSessionFieldValue(k, j[k])
			comma = ", "
		}
	}
	//SET ENDS
