...
func InsertSleepView(data []byte) (string, error) {
	var j map[string]interface{}
	if err := json.Unmarshal(data, &j); err != nil {
		logger.Error(err)
		return "", errors.FromError(errors.GO_ERROR, err)
	}
	var m models.SleepView
	...
	comma := ""
	fieldMap := models.GetSleepViewFieldMap()
	insert := bytes.NewBufferString("INSERT INTO sleep_views(")
...
func BatchInsertSleepView(data []byte) ([]string, error) {
	var j []map[string]interface{}
	if err := json.Unmarshal(data, &j); err != nil {
		logger.Error(err)
	...
	var keys []string
	fieldMap := models.GetSleepViewFieldMap()
	insert := bytes.NewBufferString("UPSERT INTO sleep_views(")
...
func buildSleepViewWhereClause(query *bytes.Buffer, or []string, and []string, in string, span []string, values map[string]interface{}) {
	...
	for _, o := range ors {
		if f, ok := fieldMap[o.Column]; ok {
			...
			values[f] = getSleepViewFieldValue(o.Column, o.Value)
			cond = "OR "
		}
	}
	...
	for _, a := range ands {
		if f, ok := fieldMap[a.Column]; ok {
			...
			values[f] = getSleepViewFieldValue(a.Column, a.Value)
		}
	}
	...
	for _, s := range spans {
		if f, ok := fieldMap[s.Column]; ok {
			...
			values["from_"+f] = getSleepViewFieldValue(s.Column, s.From)
			values["to_"+f] = getSleepViewFieldValue(s.Column, s.To)
		}
	}
	fmt.Fprint(query, "1 = 1)")
}

func getSleepViewFieldValue(c string, v interface{}) interface{} {
	typeMap := models.GetSleepViewTypeMap()
...
func findSleepViewIn(seq int64, query string, values map[string]interface{}) ([]models.SleepView, *errors.SensError) {
...
func FindSleepView(or []string, and []string, in string, span []string, limit string, column string, order string) ([]models.SleepView, *errors.SensError) {
	from := time.Now().Unix()
	seq := from % 10000
	query := bytes.NewBufferString("SELECT * FROM sleep_views WHERE ")
	fieldMap := models.GetSleepViewFieldMap()
	values := make(map[string]interface{})
	buildSleepViewWhereClause(query, or, and, in, span, values)
	if column != "" {
	...
	if strings.TrimSpace(in) == "" {
		...
		m, err := findSleepViewIn(seq, q, values)
...
func UpdateSleepViewWhere(or []string, and []string, in string, span []string, data []byte) *errors.SensError {
	fieldMap := models.GetSleepViewFieldMap()
	values := make(map[string]interface{})
	update := bytes.NewBufferString("UPDATE sleep_views SET ")
	...
	for k, _ := range j {
		if f, ok := fieldMap[k]; ok {
			...
			values["set_"+f] = getSleepViewFieldValue(k, j[k])
			comma = ", "
		}
	}
	//SET ENDS
	...
	buildSleepViewWhereClause(update, or, and, in, span, values)
