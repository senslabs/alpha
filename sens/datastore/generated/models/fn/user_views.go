...
func InsertUserView(data []byte) string {
	...
	insert := bytes.NewBufferString("INSERT INTO user_views(")
...
func BatchInsertUserView(data []byte) {
...
func buildUserViewWhereClause(query *bytes.Buffer, or []string, and []string, in string, span []string, values* []interface{}) {
...
func FindUserView(or []string, and []string, in string, span []string, limit string, column string, order string) []map[string]interface{} {
	query := bytes.NewBufferString("SELECT * FROM user_views WHERE ")
	fieldMap := models.GetUserViewFieldMap()
	var values []interface{}
	buildUserViewWhereClause(query, or, and, in, span, &values)
	if column == "" {
		column = "created_at"
	}
	...
	result := datastore.RowsToMap(r, models.GetUserViewReverseFieldMap(), models.GetUserViewTypeMap())
	return result
}

func UpdateUserViewWhere(or []string, and []string, in string, span []string, data []byte) {
	var values []interface{}
	j := types.UnmarshalMap(data)
	fieldMap := models.GetUserViewFieldMap()
	typeMap := models.GetUserViewTypeMap()
	update := bytes.NewBufferString("UPDATE user_views SET ")
	...
	buildUserViewWhereClause(update, or, and, in, span, &values)
...
func DeleteUserView(or []string, and []string, in string, span []string) int64 {
	query := bytes.NewBufferString("DELETE FROM user_views WHERE ")
	var values []interface{}
	buildUserViewWhereClause(query, or, and, in, span, &values)
	q := query.String()
	logger.Debug(q)
