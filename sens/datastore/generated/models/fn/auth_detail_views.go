...
func buildAuthDetailViewWhereClause(query *bytes.Buffer, or []string, and []string, in string, span []string, values map[string]interface{}) {
...
func findAuthDetailViewIn(seq int64, query string, values map[string]interface{}) ([]models.AuthDetailView, *errors.SensError) {
...
func FindAuthDetailView(or []string, and []string, in string, span []string, limit string, column string, order string) ([]models.AuthDetailView, *errors.SensError) {
	from := time.Now().Unix()
	seq := from % 10000
	...
	if strings.TrimSpace(in) == "" {
		...
		m, err := findAuthDetailViewIn(seq, q, values)
...
func UpdateAuthDetailViewWhere(or []string, and []string, in string, span []string, data []byte) *errors.SensError {
	fieldMap := models.GetAuthDetailViewFieldMap()
	values := make(map[string]interface{})
	...
	for k, _ := range j {
		if f, ok := fieldMap[k]; ok {
			...
			values["set_"+f] = getAuthDetailViewFieldValue(k, j[k])
			comma = ", "
		}
	}
	//SET ENDS
