...
func BatchInsertSessionEvent(data []byte) {
	...
	fmt.Fprint(insert, " ON CONFLICT(user_id,key,started_at) DO UPDATE SET (", strings.Join(fields, ", "), ") = (EXCLUDED.", strings.Join(fields, ", EXCLUDED."), ")")
...
func BatchUpsertSessionEvent(data []byte) {
	var j []map[string]interface{}
	types.Unmarshal(data, &j)
	if len(j) == 0 {
		return
...
func buildSessionEventWhereClause(query *bytes.Buffer, or []string, and []string, in string, span []string, values* []interface{}) {
...
func FindSessionEvent(or []string, and []string, in string, span []string, limit string, column string, order string) []map[string]interface{} {
	...
	result := datastore.RowsToMap(r, models.GetSessionEventReverseFieldMap(), models.GetSessionEventTypeMap())
	return result
}

func UpdateSessionEventWhere(or []string, and []string, in string, span []string, data []byte) {
	var values []interface{}
	j := types.UnmarshalMap(data)
	fieldMap := models.GetSessionEventFieldMap()
	typeMap := models.GetSessionEventTypeMap()
...
func DeleteSessionEvent(or []string, and []string, in string, span []string) int64 {
	query := bytes.NewBufferString("DELETE FROM session_events WHERE ")
	var values []interface{}
	buildSessionEventWhereClause(query, or, and, in, span, &values)
	q := query.String()
	logger.Debug(q)
