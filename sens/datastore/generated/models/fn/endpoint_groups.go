...
func InsertEndpointGroup(data []byte) (string, error) {
	var j map[string]interface{}
	if err := json.Unmarshal(data, &j); err != nil {
		logger.Error(err)
		return "", nil
	}
	var m models.EndpointGroup
	...
	comma := ""
	fieldMap := models.GetEndpointGroupFieldMap()
	insert := bytes.NewBufferString("INSERT INTO endpoint_groups(")
...
func UpdateEndpointGroup(id string, data []byte) error {
	var j map[string]interface{}
	if err := json.Unmarshal(data, &j); err != nil {
		logger.Error(err)
	...
	comma := ""
	fieldMap := models.GetEndpointGroupFieldMap()
	update := bytes.NewBufferString("UPDATE endpoint_groups SET ")
	for k, _ := range j {
		if f, ok := fieldMap[k]; ok {
...
func SelectEndpointGroup(id string) (models.EndpointGroup, *errors.SensError) {
	db := models.GetConnection()
	m := models.EndpointGroup{}
...
func FindEndpointGroup(or []string, and []string, span []string, limit string, column string, order string) ([]models.EndpointGroup, *errors.SensError) {
	...
	query := bytes.NewBufferString("SELECT * FROM endpoint_groups WHERE ")
	for _, o := range ors {
