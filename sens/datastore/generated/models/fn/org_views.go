...
func BatchUpsertOrgView(data []byte) {
	var j []map[string]interface{}
	types.Unmarshal(data, &j)
	if len(j) == 0 {
		return
	...
	fieldMap := models.GetOrgViewFieldMap()
	typeMap := models.GetOrgViewTypeMap()
	insert := bytes.NewBufferString("UPSERT INTO org_views(")
	for k, _ := range j[0] {
		if f, ok := fieldMap[k]; ok {
			fmt.Fprint(insert, comma, f)
			keys = append(keys, k)
...
func DeleteOrgView(or []string, and []string, in string, span []string) int64 {
	query := bytes.NewBufferString("DELETE FROM org_views WHERE ")
	var values []interface{}
	buildOrgViewWhereClause(query, or, and, in, span, &values)
	q := query.String()
	logger.Debug(q)
