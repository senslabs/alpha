...
func BatchInsertUserPropertie(data []byte) {
	...
	fmt.Fprint(insert, " ON CONFLICT(user_id,key) DO UPDATE SET (", strings.Join(fields, ", "), ") = (EXCLUDED.", strings.Join(fields, ", EXCLUDED."), ")")
...
func BatchUpsertUserPropertie(data []byte) {
	var j []map[string]interface{}
	types.Unmarshal(data, &j)
	if len(j) == 0 {
		return
...
func FindUserPropertie(or []string, and []string, in string, span []string, limit string, column string, order string) []map[string]interface{} {
	...
	result := datastore.RowsToMap(r, models.GetUserPropertieReverseFieldMap(), models.GetUserPropertieTypeMap())
	return result
}
...
func DeleteUserPropertie(or []string, and []string, in string, span []string) int64 {
	query := bytes.NewBufferString("DELETE FROM user_properties WHERE ")
	var values []interface{}
	buildUserPropertieWhereClause(query, or, and, in, span, &values)
	q := query.String()
	logger.Debug(q)
