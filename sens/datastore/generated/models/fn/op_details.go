...
func InsertOpDetail(data []byte) (string, error) {
	var j map[string]interface{}
	if err := json.Unmarshal(data, &j); err != nil {
		logger.Error(err)
		return "", errors.FromError(errors.GO_ERROR, err)
	}
	var m models.OpDetail
	...
	comma := ""
	fieldMap := models.GetOpDetailFieldMap()
	insert := bytes.NewBufferString("INSERT INTO op_details(")
...
func BatchInsertOpDetail(data []byte) ([]string, error) {
	var j []map[string]interface{}
	if err := json.Unmarshal(data, &j); err != nil {
		logger.Error(err)
...
func UpdateOpDetail(id string, data []byte) error {
	var j map[string]interface{}
	if err := json.Unmarshal(data, &j); err != nil {
		logger.Error(err)
		return errors.FromError(errors.GO_ERROR, err)
	...
	comma := ""
	fieldMap := models.GetOpDetailFieldMap()
	update := bytes.NewBufferString("UPDATE op_details SET ")
	for k, _ := range j {
		if f, ok := fieldMap[k]; ok {
...
func SelectOpDetail(id string) (models.OpDetail, *errors.SensError) {
	db := datastore.GetConnection()
	m := models.OpDetail{}
	if err := db.Get(&m, "SELECT * FROM op_details WHERE id = $1", id); err != nil {
		logger.Error(err)
...
func FindOpDetail(or []string, and []string, span []string, limit string, column string, order string) ([]models.OpDetail, *errors.SensError) {
	...
	query := bytes.NewBufferString("SELECT * FROM op_details WHERE ")
	for _, o := range ors {
