...
func InsertOpAuth(data []byte) (string, error) {
	...
	comma := ""
	fieldMap := models.GetOpAuthFieldMap()
	insert := bytes.NewBufferString("INSERT INTO op_auths(")
...
func BatchInsertOpAuth(data []byte) ([]string, error) {
	var j []map[string]interface{}
	if err := json.Unmarshal(data, &j); err != nil {
		logger.Error(err)
...
func UpdateOpAuth(id string, data []byte) error {
	var j map[string]interface{}
	if err := json.Unmarshal(data, &j); err != nil {
		logger.Error(err)
		return errors.FromError(errors.GO_ERROR, err)
	...
	comma := ""
	fieldMap := models.GetOpAuthFieldMap()
	update := bytes.NewBufferString("UPDATE op_auths SET ")
	for k, _ := range j {
		if f, ok := fieldMap[k]; ok {
...
func SelectOpAuth(id string) (models.OpAuth, *errors.SensError) {
	db := models.GetConnection()
	m := models.OpAuth{}
	if err := db.Get(&m, "SELECT * FROM op_auths WHERE id = $1", id); err != nil {
		logger.Error(err)
...
func FindOpAuth(or []string, and []string, span []string, limit string, column string, order string) ([]models.OpAuth, *errors.SensError) {
	...
	query := bytes.NewBufferString("SELECT * FROM op_auths WHERE ")
	for _, o := range ors {
