...
func SelectOpAuth(id string) models.OpAuth {
	db := models.GetConnection()
	m := models.OpAuth{}
	err := db.Get(&m, "SELECT * FROM op_auths WHERE id = $1", id)
	if err != nil {
		log.Println(err)
	}
	return m
...
func FindOpAuth(or []string, and []string, span []string) []models.OpAuth {
