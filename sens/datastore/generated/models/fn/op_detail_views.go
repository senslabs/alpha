...
func buildOpDetailViewWhereClause(query *bytes.Buffer, or []string, and []string, in string, span []string, values map[string]interface{}) {
	...
	for _, o := range ors {
		if f, ok := fieldMap[o.Column]; ok {
			...
			values[f] = getOpDetailViewFieldValue(o.Column, o.Value)
			cond = "OR "
		}
	}
	...
	for _, a := range ands {
		if f, ok := fieldMap[a.Column]; ok {
			...
			values[f] = getOpDetailViewFieldValue(a.Column, a.Value)
		}
	}
	...
	for _, s := range spans {
		if f, ok := fieldMap[s.Column]; ok {
			...
			values["from_"+f] = getOpDetailViewFieldValue(s.Column, s.From)
			values["to_"+f] = getOpDetailViewFieldValue(s.Column, s.To)
		}
	}
...
func findOpDetailViewIn(seq int64, query string, values map[string]interface{}) ([]models.OpDetailView, *errors.SensError) {
...
func FindOpDetailView(or []string, and []string, in string, span []string, limit string, column string, order string) ([]models.OpDetailView, *errors.SensError) {
	from := time.Now().Unix()
	seq := from % 10000
	query := bytes.NewBufferString("SELECT * FROM op_detail_views WHERE ")
	fieldMap := models.GetOpDetailViewFieldMap()
	values := make(map[string]interface{})
	buildOpDetailViewWhereClause(query, or, and, in, span, values)
	if column != "" {
	...
	if strings.TrimSpace(in) == "" {
		...
		m, err := findOpDetailViewIn(seq, q, values)
...
func UpdateOpDetailViewWhere(or []string, and []string, in string, span []string, data []byte) *errors.SensError {
	fieldMap := models.GetOpDetailViewFieldMap()
	values := make(map[string]interface{})
	...
	for k, _ := range j {
		if f, ok := fieldMap[k]; ok {
			...
			values["set_"+f] = getOpDetailViewFieldValue(k, j[k])
			comma = ", "
		}
	}
	//SET ENDS
	...
	buildOpDetailViewWhereClause(update, or, and, in, span, values)
