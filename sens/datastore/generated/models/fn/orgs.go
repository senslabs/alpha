...
func BatchInsertOrg(data []byte) {
	...
	fmt.Fprint(insert, " ON CONFLICT(org_id) DO UPDATE SET (", strings.Join(fields, ", "), ") = (EXCLUDED.", strings.Join(fields, ", EXCLUDED."), ")")
...
func UpdateOrg(id string, data []byte) {
	...
	fmt.Fprint(update, " WHERE org_id = $", phi)
...
func SelectOrg(id string) map[string]interface{} {
	...
	stmt, err := db.Prepare("SELECT * FROM orgs WHERE org_id = $1")
	defer stmt.Close()
	errors.Pie(err)
...
func DeleteOrg(or []string, and []string, in string, span []string) int64 {
	query := bytes.NewBufferString("DELETE FROM orgs WHERE ")
	var values []interface{}
	buildOrgWhereClause(query, or, and, in, span, &values)
	q := query.String()
	logger.Debug(q)
