...
func UpdateOrg(id string, data []byte) error {
	...
	fmt.Fprint(update, " WHERE org_id = :org_id")
...
func SelectOrg(id string) (models.Org, *errors.SensError) {
	db := datastore.GetConnection()
	m := models.Org{}
	if err := db.Get(&m, "SELECT * FROM orgs WHERE org_id = $1", id); err != nil {
		logger.Error(err)
...
func buildOrgWhereClause(query *bytes.Buffer, or []string, and []string, in string, span []string, values map[string]interface{}) {
	...
	for _, o := range ors {
		if f, ok := fieldMap[o.Column]; ok {
			...
			values[f] = getOrgFieldValue(o.Column, o.Value)
			cond = "OR "
		}
	}
	...
	for _, a := range ands {
		if f, ok := fieldMap[a.Column]; ok {
			...
			values[f] = getOrgFieldValue(a.Column, a.Value)
		}
	}
	...
	for _, s := range spans {
		if f, ok := fieldMap[s.Column]; ok {
			...
			values["from_"+f] = getOrgFieldValue(s.Column, s.From)
			values["to_"+f] = getOrgFieldValue(s.Column, s.To)
		}
	}
	fmt.Fprint(query, "1 = 1)")
}

func getOrgFieldValue(c string, v interface{}) interface{} {
...
func findOrgIn(seq int64, query string, values map[string]interface{}) ([]models.Org, *errors.SensError) {
...
func UpdateOrgWhere(or []string, and []string, in string, span []string, data []byte) *errors.SensError {
	fieldMap := models.GetOrgFieldMap()
	values := make(map[string]interface{})
	update := bytes.NewBufferString("UPDATE orgs SET ")
	...
	for k, _ := range j {
		if f, ok := fieldMap[k]; ok {
			...
			values["set_"+f] = getOrgFieldValue(k, j[k])
			comma = ", "
		}
	}
	//SET ENDS
