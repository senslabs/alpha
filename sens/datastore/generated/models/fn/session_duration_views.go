...
func BatchUpsertSessionDurationView(data []byte) {
	var j []map[string]interface{}
	types.Unmarshal(data, &j)
	if len(j) == 0 {
		return
	...
	fieldMap := models.GetSessionDurationViewFieldMap()
	typeMap := models.GetSessionDurationViewTypeMap()
	insert := bytes.NewBufferString("UPSERT INTO session_duration_views(")
	for k, _ := range j[0] {
		if f, ok := fieldMap[k]; ok {
			fmt.Fprint(insert, comma, f)
			keys = append(keys, k)
...
func DeleteSessionDurationView(or []string, and []string, in string, span []string) int64 {
	query := bytes.NewBufferString("DELETE FROM session_duration_views WHERE ")
	var values []interface{}
	buildSessionDurationViewWhereClause(query, or, and, in, span, &values)
	q := query.String()
	logger.Debug(q)
