...
func InsertOpEndpoint(data []byte) string {
...
func BatchInsertOpEndpoint(data []byte) {
	...
	fmt.Fprint(insert, " ON CONFLICT(op_id,endpoint_id) DO UPDATE SET (", strings.Join(fields, ", "), ") = (EXCLUDED.", strings.Join(fields, ", EXCLUDED."), ")")
...
func BatchUpsertOpEndpoint(data []byte) {
	var j []map[string]interface{}
	types.Unmarshal(data, &j)
	if len(j) == 0 {
		return
...
func buildOpEndpointWhereClause(query *bytes.Buffer, or []string, and []string, in string, span []string, values* []interface{}) {
...
func FindOpEndpoint(or []string, and []string, in string, span []string, limit string, column string, order string) []map[string]interface{} {
	query := bytes.NewBufferString("SELECT * FROM op_endpoints WHERE ")
	fieldMap := models.GetOpEndpointFieldMap()
	var values []interface{}
	buildOpEndpointWhereClause(query, or, and, in, span, &values)
	if column == "" {
		column = "created_at"
	}
	...
	result := datastore.RowsToMap(r, models.GetOpEndpointReverseFieldMap(), models.GetOpEndpointTypeMap())
	return result
}

func UpdateOpEndpointWhere(or []string, and []string, in string, span []string, data []byte) {
	var values []interface{}
	j := types.UnmarshalMap(data)
	fieldMap := models.GetOpEndpointFieldMap()
	typeMap := models.GetOpEndpointTypeMap()
	...
	buildOpEndpointWhereClause(update, or, and, in, span, &values)
...
func DeleteOpEndpoint(or []string, and []string, in string, span []string) int64 {
	query := bytes.NewBufferString("DELETE FROM op_endpoints WHERE ")
	var values []interface{}
	buildOpEndpointWhereClause(query, or, and, in, span, &values)
	q := query.String()
	logger.Debug(q)
