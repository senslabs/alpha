...
func buildOpEndpointWhereClause(query *bytes.Buffer, or []string, and []string, in string, span []string, values map[string]interface{}) {
...
func findOpEndpointIn(seq int64, query string, values map[string]interface{}) ([]models.OpEndpoint, *errors.SensError) {
...
func FindOpEndpoint(or []string, and []string, in string, span []string, limit string, column string, order string) ([]models.OpEndpoint, *errors.SensError) {
	from := time.Now().Unix()
	seq := from % 10000
	...
	if strings.TrimSpace(in) == "" {
		...
		m, err := findOpEndpointIn(seq, q, values)
...
func UpdateOpEndpointWhere(or []string, and []string, in string, span []string, data []byte) *errors.SensError {
	fieldMap := models.GetOpEndpointFieldMap()
	values := make(map[string]interface{})
	...
	for k, _ := range j {
		if f, ok := fieldMap[k]; ok {
			...
			values["set_"+f] = getOpEndpointFieldValue(k, j[k])
			comma = ", "
		}
	}
	//SET ENDS
