...
func UpdateOrgUser(id string, data []byte) error {
	var j map[string]interface{}
	if err := json.Unmarshal(data, &j); err != nil {
		logger.Error(err)
...
func SelectOrgUser(id string) (models.OrgUser, *errors.SensError) {
	db := models.GetConnection()
	m := models.OrgUser{}
	if err := db.Get(&m, "SELECT * FROM org_users WHERE id = $1", id); err != nil {
		logger.Error(err)
