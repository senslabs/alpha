...
func InsertDevice(data []byte) (string, error) {
	...
	fmt.Fprint(insert, " returning device_id")
...
func UpdateDevice(id string, data []byte) error {
	...
	fmt.Fprint(update, " WHERE device_id = :device_id")
...
func SelectDevice(id string) (models.Device, *errors.SensError) {
	db := datastore.GetConnection()
	m := models.Device{}
	if err := db.Get(&m, "SELECT * FROM devices WHERE device_id = $1", id); err != nil {
		logger.Error(err)
...
func buildDeviceWhereClause(query *bytes.Buffer, or []string, and []string, span []string, values map[string]interface{}) {
...
func FindDevice(or []string, and []string, span []string, limit string, column string, order string) ([]models.Device, *errors.SensError) {
	...
	buildDeviceWhereClause(query, or, and, span, values)
	if column != "" {
...
func UpdateDeviceWhere(or []string, and []string, span []string, data []byte) *errors.SensError {
	fieldMap := models.GetDeviceFieldMap()
	values := make(map[string]interface{})
	update := bytes.NewBufferString("UPDATE devices SET ")
	...
	buildDeviceWhereClause(update, or, and, span, values)
