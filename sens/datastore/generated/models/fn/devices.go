...
func InsertDevice(data []byte) string {
...
func BatchInsertDevice(data []byte) {
	...
	fmt.Fprint(insert, " ON CONFLICT(device_id,created_at) DO UPDATE SET (", strings.Join(fields, ", "), ") = (EXCLUDED.", strings.Join(fields, ", EXCLUDED."), ")")
...
func BatchUpsertDevice(data []byte) {
	var j []map[string]interface{}
	types.Unmarshal(data, &j)
	if len(j) == 0 {
		return
...
func buildDeviceWhereClause(query *bytes.Buffer, or []string, and []string, in string, span []string, values* []interface{}) {
...
func FindDevice(or []string, and []string, in string, span []string, limit string, column string, order string) []map[string]interface{} {
	...
	result := datastore.RowsToMap(r, models.GetDeviceReverseFieldMap(), models.GetDeviceTypeMap())
	return result
}

func UpdateDeviceWhere(or []string, and []string, in string, span []string, data []byte) {
	var values []interface{}
	j := types.UnmarshalMap(data)
	fieldMap := models.GetDeviceFieldMap()
	typeMap := models.GetDeviceTypeMap()
...
func DeleteDevice(or []string, and []string, in string, span []string) int64 {
	query := bytes.NewBufferString("DELETE FROM devices WHERE ")
	var values []interface{}
	buildDeviceWhereClause(query, or, and, in, span, &values)
	q := query.String()
	logger.Debug(q)
