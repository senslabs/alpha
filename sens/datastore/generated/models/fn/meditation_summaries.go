...
func InsertMeditationSummarie(data []byte) (string, error) {
	var j map[string]interface{}
	if err := json.Unmarshal(data, &j); err != nil {
		logger.Error(err)
		return "", errors.FromError(errors.GO_ERROR, err)
	}
	var m models.MeditationSummarie
...
func BatchInsertMeditationSummarie(data []byte) ([]string, error) {
	var j []map[string]interface{}
	if err := json.Unmarshal(data, &j); err != nil {
		logger.Error(err)
	...
	var keys []string
	fieldMap := models.GetMeditationSummarieFieldMap()
	insert := bytes.NewBufferString("UPSERT INTO meditation_summaries(")
...
func buildMeditationSummarieWhereClause(query *bytes.Buffer, or []string, and []string, span []string, values map[string]interface{}) {
	...
	for _, o := range ors {
		if f, ok := fieldMap[o.Column]; ok {
			...
			values[f] = getMeditationSummarieFieldValue(o.Column, o.Value)
			cond = "OR "
		}
	}
	...
	for _, a := range ands {
		if f, ok := fieldMap[a.Column]; ok {
			...
			values[f] = getMeditationSummarieFieldValue(a.Column, a.Value)
		}
	}
	for _, s := range spans {
		if f, ok := fieldMap[s.Column]; ok {
			fmt.Fprint(query, fmt.Sprintf("%s >= :from_%s AND %s <= :to_%s AND ", f, f, f, f))
			values["from_"+f] = getMeditationSummarieFieldValue(s.Column, s.From)
			values["to_"+f] = getMeditationSummarieFieldValue(s.Column, s.To)
		}
	}
	fmt.Fprint(query, "1 = 1)")
}

func getMeditationSummarieFieldValue(c string, v interface{}) interface{} {
	// typeMap := models.GetAuthTypeMap()
	return v
}

func FindMeditationSummarie(or []string, and []string, span []string, limit string, column string, order string) ([]models.MeditationSummarie, *errors.SensError) {
	query := bytes.NewBufferString("SELECT * FROM meditation_summaries WHERE ")
	fieldMap := models.GetMeditationSummarieFieldMap()
	values := make(map[string]interface{})
	buildMeditationSummarieWhereClause(query, or, and, span, values)
	if column != "" {
...
func UpdateMeditationSummarieWhere(or []string, and []string, span []string, data []byte) *errors.SensError {
	fieldMap := models.GetMeditationSummarieFieldMap()
	values := make(map[string]interface{})
	...
	buildMeditationSummarieWhereClause(update, or, and, span, values)
