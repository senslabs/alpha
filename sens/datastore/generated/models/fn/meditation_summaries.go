...
func buildMeditationSummarieWhereClause(query *bytes.Buffer, or []string, and []string, in string, span []string, values map[string]interface{}) {
...
func findMeditationSummarieIn(seq int64, query string, values map[string]interface{}) ([]models.MeditationSummarie, *errors.SensError) {
...
func FindMeditationSummarie(or []string, and []string, in string, span []string, limit string, column string, order string) ([]models.MeditationSummarie, *errors.SensError) {
	...
	if strings.TrimSpace(in) == "" {
		...
		m, err := findMeditationSummarieIn(seq, q, values)
...
func UpdateMeditationSummarieWhere(or []string, and []string, in string, span []string, data []byte) *errors.SensError {
	fieldMap := models.GetMeditationSummarieFieldMap()
	values := make(map[string]interface{})
	...
	for k, _ := range j {
		if f, ok := fieldMap[k]; ok {
			...
			values["set_"+f] = getMeditationSummarieFieldValue(k, j[k])
			comma = ", "
		}
	}
	//SET ENDS
