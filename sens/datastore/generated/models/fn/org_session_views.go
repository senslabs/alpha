...
func BatchUpsertOrgSessionView(data []byte) {
	var j []map[string]interface{}
	types.Unmarshal(data, &j)
	if len(j) == 0 {
		return
	...
	fieldMap := models.GetOrgSessionViewFieldMap()
	typeMap := models.GetOrgSessionViewTypeMap()
	insert := bytes.NewBufferString("UPSERT INTO org_session_views(")
	for k, _ := range j[0] {
		if f, ok := fieldMap[k]; ok {
			fmt.Fprint(insert, comma, f)
			keys = append(keys, k)
...
func DeleteOrgSessionView(or []string, and []string, in string, span []string) int64 {
	query := bytes.NewBufferString("DELETE FROM org_session_views WHERE ")
	var values []interface{}
	buildOrgSessionViewWhereClause(query, or, and, in, span, &values)
	q := query.String()
	logger.Debug(q)
