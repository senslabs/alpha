...
func buildSessionPropertieWhereClause(query *bytes.Buffer, or []string, and []string, in string, span []string, values map[string]interface{}) {
...
func findSessionPropertieIn(seq int64, query string, values map[string]interface{}) ([]models.SessionPropertie, *errors.SensError) {
...
func FindSessionPropertie(or []string, and []string, in string, span []string, limit string, column string, order string) ([]models.SessionPropertie, *errors.SensError) {
	from := time.Now().Unix()
	seq := from % 10000
	query := bytes.NewBufferString("SELECT * FROM session_properties WHERE ")
	fieldMap := models.GetSessionPropertieFieldMap()
	values := make(map[string]interface{})
	buildSessionPropertieWhereClause(query, or, and, in, span, values)
	if column != "" {
	...
	if strings.TrimSpace(in) == "" {
		...
		m, err := findSessionPropertieIn(seq, q, values)
...
func UpdateSessionPropertieWhere(or []string, and []string, in string, span []string, data []byte) *errors.SensError {
	fieldMap := models.GetSessionPropertieFieldMap()
	values := make(map[string]interface{})
	...
	for k, _ := range j {
		if f, ok := fieldMap[k]; ok {
			...
			values["set_"+f] = getSessionPropertieFieldValue(k, j[k])
			comma = ", "
		}
	}
	//SET ENDS
	...
	buildSessionPropertieWhereClause(update, or, and, in, span, values)
