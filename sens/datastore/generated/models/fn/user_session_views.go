...
func InsertUserSessionView(data []byte) (string, error) {
	var j map[string]interface{}
	if err := json.Unmarshal(data, &j); err != nil {
		logger.Error(err)
		return "", errors.FromError(errors.GO_ERROR, err)
	}
	var m models.UserSessionView
...
func BatchInsertUserSessionView(data []byte) ([]string, error) {
	var j []map[string]interface{}
	if err := json.Unmarshal(data, &j); err != nil {
		logger.Error(err)
	...
	var keys []string
	fieldMap := models.GetUserSessionViewFieldMap()
	insert := bytes.NewBufferString("UPSERT INTO user_session_views(")
...
func buildUserSessionViewWhereClause(query *bytes.Buffer, or []string, and []string, in string, span []string, values map[string]interface{}) {
	...
	for _, o := range ors {
		if f, ok := fieldMap[o.Column]; ok {
			...
			values[f] = getUserSessionViewFieldValue(o.Column, o.Value)
			cond = "OR "
		}
	}
	...
	for _, a := range ands {
		if f, ok := fieldMap[a.Column]; ok {
			...
			values[f] = getUserSessionViewFieldValue(a.Column, a.Value)
		}
	}
	...
	for _, s := range spans {
		if f, ok := fieldMap[s.Column]; ok {
			...
			values["from_"+f] = getUserSessionViewFieldValue(s.Column, s.From)
			values["to_"+f] = getUserSessionViewFieldValue(s.Column, s.To)
		}
	}
	fmt.Fprint(query, "1 = 1)")
}

func getUserSessionViewFieldValue(c string, v interface{}) interface{} {
...
func findUserSessionViewIn(seq int64, query string, values map[string]interface{}) ([]models.UserSessionView, *errors.SensError) {
...
func FindUserSessionView(or []string, and []string, in string, span []string, limit string, column string, order string) ([]models.UserSessionView, *errors.SensError) {
	from := time.Now().Unix()
	seq := from % 10000
	...
	if strings.TrimSpace(in) == "" {
		...
		m, err := findUserSessionViewIn(seq, q, values)
...
func UpdateUserSessionViewWhere(or []string, and []string, in string, span []string, data []byte) *errors.SensError {
	fieldMap := models.GetUserSessionViewFieldMap()
	values := make(map[string]interface{})
	...
	for k, _ := range j {
		if f, ok := fieldMap[k]; ok {
			...
			values["set_"+f] = getUserSessionViewFieldValue(k, j[k])
			comma = ", "
		}
	}
	//SET ENDS
