...
func buildUserSessionViewWhereClause(query *bytes.Buffer, or []string, and []string, span []string, values map[string]interface{}) {
...
func FindUserSessionView(or []string, and []string, span []string, limit string, column string, order string) ([]models.UserSessionView, *errors.SensError) {
	query := bytes.NewBufferString("SELECT * FROM user_session_views WHERE ")
	fieldMap := models.GetUserSessionViewFieldMap()
	values := make(map[string]interface{})
	buildUserSessionViewWhereClause(query, or, and, span, values)
	if column != "" {
...
func UpdateUserSessionViewWhere(or []string, and []string, span []string, data []byte) *errors.SensError {
	fieldMap := models.GetUserSessionViewFieldMap()
	values := make(map[string]interface{})
	...
	buildUserSessionViewWhereClause(update, or, and, span, values)
