...
func BatchUpsertApiKey(data []byte) {
	var j []map[string]interface{}
	types.Unmarshal(data, &j)
	if len(j) == 0 {
		return
...
func DeleteApiKey(or []string, and []string, in string, span []string) int64 {
	query := bytes.NewBufferString("DELETE FROM api_keys WHERE ")
	var values []interface{}
	buildApiKeyWhereClause(query, or, and, in, span, &values)
	q := query.String()
	logger.Debug(q)
