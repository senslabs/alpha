...
func InsertSurveyAnswer(data []byte) string {
...
func BatchInsertSurveyAnswer(data []byte) {
	...
	fmt.Fprint(insert, " ON CONFLICT(survey_answer_id) DO UPDATE SET (", strings.Join(fields, ", "), ") = (EXCLUDED.", strings.Join(fields, ", EXCLUDED."), ")")
...
func BatchUpsertSurveyAnswer(data []byte) {
	var j []map[string]interface{}
	types.Unmarshal(data, &j)
	if len(j) == 0 {
		return
...
func UpdateSurveyAnswer(id string, data []byte) {
	var j map[string]interface{}
	types.Unmarshal(data, &j)
	...
	fmt.Fprint(update, " WHERE survey_answer_id = $", phi)
...
func SelectSurveyAnswer(id string) map[string]interface{} {
	db := datastore.GetConnection()

	stmt, err := db.Prepare("SELECT * FROM survey_answers WHERE survey_answer_id = $1")
	defer stmt.Close()
	errors.Pie(err)
	...
	result := datastore.RowsToMap(r, models.GetSurveyAnswerReverseFieldMap(), models.GetSurveyAnswerTypeMap())
	if len(result) == 0 {
...
func buildSurveyAnswerWhereClause(query *bytes.Buffer, or []string, and []string, in string, span []string, values* []interface{}) {
...
func FindSurveyAnswer(or []string, and []string, in string, span []string, limit string, column string, order string) []map[string]interface{} {
...
func UpdateSurveyAnswerWhere(or []string, and []string, in string, span []string, data []byte) {
	var values []interface{}
	j := types.UnmarshalMap(data)
	fieldMap := models.GetSurveyAnswerFieldMap()
	typeMap := models.GetSurveyAnswerTypeMap()
...
func DeleteSurveyAnswer(or []string, and []string, in string, span []string) int64 {
	query := bytes.NewBufferString("DELETE FROM survey_answers WHERE ")
	var values []interface{}
	buildSurveyAnswerWhereClause(query, or, and, in, span, &values)
	q := query.String()
	logger.Debug(q)
