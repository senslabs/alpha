...
func InsertOrgEndpointAccessGroup(data []byte) string {
...
func BatchInsertOrgEndpointAccessGroup(data []byte) {
	...
	fmt.Fprint(insert, " ON CONFLICT(org_id,access_group) DO UPDATE SET (", strings.Join(fields, ", "), ") = (EXCLUDED.", strings.Join(fields, ", EXCLUDED."), ")")
...
func BatchUpsertOrgEndpointAccessGroup(data []byte) {
	var j []map[string]interface{}
	types.Unmarshal(data, &j)
	if len(j) == 0 {
		return
...
func buildOrgEndpointAccessGroupWhereClause(query *bytes.Buffer, or []string, and []string, in string, span []string, values* []interface{}) {
...
func FindOrgEndpointAccessGroup(or []string, and []string, in string, span []string, limit string, column string, order string) []map[string]interface{} {
	query := bytes.NewBufferString("SELECT * FROM org_endpoint_access_groups WHERE ")
	fieldMap := models.GetOrgEndpointAccessGroupFieldMap()
	var values []interface{}
	buildOrgEndpointAccessGroupWhereClause(query, or, and, in, span, &values)
	if column == "" {
		column = "created_at"
	}
...
func UpdateOrgEndpointAccessGroupWhere(or []string, and []string, in string, span []string, data []byte) {
	...
	buildOrgEndpointAccessGroupWhereClause(update, or, and, in, span, &values)
...
func DeleteOrgEndpointAccessGroup(or []string, and []string, in string, span []string) int64 {
	query := bytes.NewBufferString("DELETE FROM org_endpoint_access_groups WHERE ")
	var values []interface{}
	buildOrgEndpointAccessGroupWhereClause(query, or, and, in, span, &values)
	q := query.String()
	logger.Debug(q)
