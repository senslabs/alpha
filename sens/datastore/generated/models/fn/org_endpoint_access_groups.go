...
func buildOrgEndpointAccessGroupWhereClause(query *bytes.Buffer, or []string, and []string, in string, span []string, values map[string]interface{}) {
...
func findOrgEndpointAccessGroupIn(seq int64, query string, values map[string]interface{}) ([]models.OrgEndpointAccessGroup, *errors.SensError) {
...
func FindOrgEndpointAccessGroup(or []string, and []string, in string, span []string, limit string, column string, order string) ([]models.OrgEndpointAccessGroup, *errors.SensError) {
	from := time.Now().Unix()
	seq := from % 10000
	...
	if strings.TrimSpace(in) == "" {
		...
		m, err := findOrgEndpointAccessGroupIn(seq, q, values)
...
func UpdateOrgEndpointAccessGroupWhere(or []string, and []string, in string, span []string, data []byte) *errors.SensError {
	fieldMap := models.GetOrgEndpointAccessGroupFieldMap()
	values := make(map[string]interface{})
	...
	for k, _ := range j {
		if f, ok := fieldMap[k]; ok {
			...
			values["set_"+f] = getOrgEndpointAccessGroupFieldValue(k, j[k])
			comma = ", "
		}
	}
	//SET ENDS
