...
func InsertSessionSetting(data []byte) (string, error) {
	var j map[string]interface{}
	if err := json.Unmarshal(data, &j); err != nil {
		logger.Error(err)
		return "", errors.FromError(errors.GO_ERROR, err)
	}
	var m models.SessionSetting
	...
	fmt.Fprint(insert, " returning session_setting_id")
...
func BatchInsertSessionSetting(data []byte) ([]string, error) {
	var j []map[string]interface{}
	if err := json.Unmarshal(data, &j); err != nil {
		logger.Error(err)
...
func UpdateSessionSetting(id string, data []byte) error {
	var j map[string]interface{}
	if err := json.Unmarshal(data, &j); err != nil {
		logger.Error(err)
		return errors.FromError(errors.GO_ERROR, err)
	...
	comma := ""
	fieldMap := models.GetSessionSettingFieldMap()
	update := bytes.NewBufferString("UPDATE session_settings SET ")
	for k, _ := range j {
		if f, ok := fieldMap[k]; ok {
	...
	fmt.Fprint(update, " WHERE session_setting_id = :session_setting_id")
...
func SelectSessionSetting(id string) (models.SessionSetting, *errors.SensError) {
	db := datastore.GetConnection()
	m := models.SessionSetting{}
	if err := db.Get(&m, "SELECT * FROM session_settings WHERE session_setting_id = $1", id); err != nil {
		logger.Error(err)
...
func buildSessionSettingWhereClause(query *bytes.Buffer, or []string, and []string, in string, span []string, values map[string]interface{}) {
	...
	for _, o := range ors {
		if f, ok := fieldMap[o.Column]; ok {
			...
			values[f] = getSessionSettingFieldValue(o.Column, o.Value)
			cond = "OR "
		}
	}
	...
	for _, a := range ands {
		if f, ok := fieldMap[a.Column]; ok {
			...
			values[f] = getSessionSettingFieldValue(a.Column, a.Value)
		}
	}
	...
	for _, s := range spans {
		if f, ok := fieldMap[s.Column]; ok {
			...
			values["from_"+f] = getSessionSettingFieldValue(s.Column, s.From)
			values["to_"+f] = getSessionSettingFieldValue(s.Column, s.To)
		}
	}
	fmt.Fprint(query, "1 = 1)")
}

func getSessionSettingFieldValue(c string, v interface{}) interface{} {
...
func findSessionSettingIn(seq int64, query string, values map[string]interface{}) ([]models.SessionSetting, *errors.SensError) {
...
func FindSessionSetting(or []string, and []string, in string, span []string, limit string, column string, order string) ([]models.SessionSetting, *errors.SensError) {
	from := time.Now().Unix()
	seq := from % 10000
	query := bytes.NewBufferString("SELECT * FROM session_settings WHERE ")
	fieldMap := models.GetSessionSettingFieldMap()
	values := make(map[string]interface{})
	buildSessionSettingWhereClause(query, or, and, in, span, values)
	if column != "" {
	...
	if strings.TrimSpace(in) == "" {
		...
		m, err := findSessionSettingIn(seq, q, values)
...
func UpdateSessionSettingWhere(or []string, and []string, in string, span []string, data []byte) *errors.SensError {
	fieldMap := models.GetSessionSettingFieldMap()
	values := make(map[string]interface{})
	update := bytes.NewBufferString("UPDATE session_settings SET ")
	...
	for k, _ := range j {
		if f, ok := fieldMap[k]; ok {
			...
			values["set_"+f] = getSessionSettingFieldValue(k, j[k])
			comma = ", "
		}
	}
	//SET ENDS
	...
	buildSessionSettingWhereClause(update, or, and, in, span, values)
