...
func InsertSessionSetting(data []byte) string {
...
func BatchInsertSessionSetting(data []byte) {
...
func BatchUpsertSessionSetting(data []byte) {
	var j []map[string]interface{}
	types.Unmarshal(data, &j)
	if len(j) == 0 {
		return
...
func UpdateSessionSetting(id string, data []byte) {
	var j map[string]interface{}
	types.Unmarshal(data, &j)
	...
	fmt.Fprint(update, " WHERE session_setting_id = $", phi)
...
func SelectSessionSetting(id string) map[string]interface{} {
	db := datastore.GetConnection()

	stmt, err := db.Prepare("SELECT * FROM session_settings WHERE session_setting_id = $1")
	defer stmt.Close()
	errors.Pie(err)
	...
	result := datastore.RowsToMap(r, models.GetSessionSettingReverseFieldMap(), models.GetSessionSettingTypeMap())
	if len(result) == 0 {
...
func buildSessionSettingWhereClause(query *bytes.Buffer, or []string, and []string, in string, span []string, values* []interface{}) {
...
func FindSessionSetting(or []string, and []string, in string, span []string, limit string, column string, order string) []map[string]interface{} {
...
func UpdateSessionSettingWhere(or []string, and []string, in string, span []string, data []byte) {
	var values []interface{}
	j := types.UnmarshalMap(data)
	fieldMap := models.GetSessionSettingFieldMap()
	typeMap := models.GetSessionSettingTypeMap()
...
func DeleteSessionSetting(or []string, and []string, in string, span []string) int64 {
	query := bytes.NewBufferString("DELETE FROM session_settings WHERE ")
	var values []interface{}
	buildSessionSettingWhereClause(query, or, and, in, span, &values)
	q := query.String()
	logger.Debug(q)
