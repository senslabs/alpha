...
func buildUserAlertViewWhereClause(query *bytes.Buffer, or []string, and []string, in string, span []string, values map[string]interface{}) {
...
func findUserAlertViewIn(seq int64, query string, values map[string]interface{}) ([]models.UserAlertView, *errors.SensError) {
...
func FindUserAlertView(or []string, and []string, in string, span []string, limit string, column string, order string) ([]models.UserAlertView, *errors.SensError) {
	...
	if strings.TrimSpace(in) == "" {
		...
		m, err := findUserAlertViewIn(seq, q, values)
...
func UpdateUserAlertViewWhere(or []string, and []string, in string, span []string, data []byte) *errors.SensError {
	fieldMap := models.GetUserAlertViewFieldMap()
	values := make(map[string]interface{})
	...
	for k, _ := range j {
		if f, ok := fieldMap[k]; ok {
			...
			values["set_"+f] = getUserAlertViewFieldValue(k, j[k])
			comma = ", "
		}
	}
	//SET ENDS
