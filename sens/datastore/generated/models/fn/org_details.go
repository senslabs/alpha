...
func InsertOrgDetail(data []byte) (string, error) {
	var j map[string]interface{}
	if err := json.Unmarshal(data, &j); err != nil {
		logger.Error(err)
		return "", errors.FromError(errors.GO_ERROR, err)
	}
	var m models.OrgDetail
	...
	comma := ""
	fieldMap := models.GetOrgDetailFieldMap()
	insert := bytes.NewBufferString("INSERT INTO org_details(")
...
func BatchInsertOrgDetail(data []byte) ([]string, error) {
	var j []map[string]interface{}
	if err := json.Unmarshal(data, &j); err != nil {
		logger.Error(err)
...
func UpdateOrgDetail(id string, data []byte) error {
	var j map[string]interface{}
	if err := json.Unmarshal(data, &j); err != nil {
		logger.Error(err)
		return errors.FromError(errors.GO_ERROR, err)
	...
	comma := ""
	fieldMap := models.GetOrgDetailFieldMap()
	update := bytes.NewBufferString("UPDATE org_details SET ")
	for k, _ := range j {
		if f, ok := fieldMap[k]; ok {
...
func SelectOrgDetail(id string) (models.OrgDetail, *errors.SensError) {
	db := datastore.GetConnection()
	m := models.OrgDetail{}
	if err := db.Get(&m, "SELECT * FROM org_details WHERE id = $1", id); err != nil {
		logger.Error(err)
...
func FindOrgDetail(or []string, and []string, span []string, limit string, column string, order string) ([]models.OrgDetail, *errors.SensError) {
	...
	query := bytes.NewBufferString("SELECT * FROM org_details WHERE ")
	for _, o := range ors {
