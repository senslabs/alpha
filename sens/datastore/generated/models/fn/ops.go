...
func buildOpWhereClause(query *bytes.Buffer, or []string, and []string, span []string, values map[string]interface{}) {
...
func FindOp(or []string, and []string, span []string, limit string, column string, order string) ([]models.Op, *errors.SensError) {
	query := bytes.NewBufferString("SELECT * FROM ops WHERE ")
	fieldMap := models.GetOpFieldMap()
	values := make(map[string]interface{})
	buildOpWhereClause(query, or, and, span, values)
	if column != "" {
...
func UpdateOpWhere(or []string, and []string, span []string, data []byte) *errors.SensError {
	fieldMap := models.GetOpFieldMap()
	values := make(map[string]interface{})
	update := bytes.NewBufferString("UPDATE ops SET ")
	...
	buildOpWhereClause(update, or, and, span, values)
