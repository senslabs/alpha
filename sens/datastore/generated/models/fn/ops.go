...
func BatchInsertOp(data []byte) ([]string, error) {
	var j []map[string]interface{}
	if err := json.Unmarshal(data, &j); err != nil {
		logger.Error(err)
...
func buildOpWhereClause(query *bytes.Buffer, or []string, and []string, in string, span []string, values map[string]interface{}) {
	...
	for _, o := range ors {
		if f, ok := fieldMap[o.Column]; ok {
			...
			values[f] = getOpFieldValue(o.Column, o.Value)
			cond = "OR "
		}
	}
	...
	for _, a := range ands {
		if f, ok := fieldMap[a.Column]; ok {
			...
			values[f] = getOpFieldValue(a.Column, a.Value)
		}
	}
	...
	for _, s := range spans {
		if f, ok := fieldMap[s.Column]; ok {
			...
			values["from_"+f] = getOpFieldValue(s.Column, s.From)
			values["to_"+f] = getOpFieldValue(s.Column, s.To)
		}
	}
	fmt.Fprint(query, "1 = 1)")
}

func getOpFieldValue(c string, v interface{}) interface{} {
	typeMap := models.GetOpTypeMap()
...
func findOpIn(seq int64, query string, values map[string]interface{}) ([]models.Op, *errors.SensError) {
...
func FindOp(or []string, and []string, in string, span []string, limit string, column string, order string) ([]models.Op, *errors.SensError) {
	from := time.Now().Unix()
	seq := from % 10000
	query := bytes.NewBufferString("SELECT * FROM ops WHERE ")
	fieldMap := models.GetOpFieldMap()
	values := make(map[string]interface{})
	buildOpWhereClause(query, or, and, in, span, values)
	if column != "" {
...
func UpdateOpWhere(or []string, and []string, in string, span []string, data []byte) *errors.SensError {
	fieldMap := models.GetOpFieldMap()
	values := make(map[string]interface{})
	update := bytes.NewBufferString("UPDATE ops SET ")
	...
	for k, _ := range j {
		if f, ok := fieldMap[k]; ok {
			...
			values["set_"+f] = getOpFieldValue(k, j[k])
			comma = ", "
		}
	}
	//SET ENDS
	...
	buildOpWhereClause(update, or, and, in, span, values)
