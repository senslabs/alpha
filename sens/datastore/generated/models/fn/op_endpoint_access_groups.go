...
func buildOpEndpointAccessGroupWhereClause(query *bytes.Buffer, or []string, and []string, in string, span []string, values map[string]interface{}) {
...
func findOpEndpointAccessGroupIn(seq int64, query string, values map[string]interface{}) ([]models.OpEndpointAccessGroup, *errors.SensError) {
...
func FindOpEndpointAccessGroup(or []string, and []string, in string, span []string, limit string, column string, order string) ([]models.OpEndpointAccessGroup, *errors.SensError) {
	from := time.Now().Unix()
	seq := from % 10000
	...
	if strings.TrimSpace(in) == "" {
		...
		m, err := findOpEndpointAccessGroupIn(seq, q, values)
...
func UpdateOpEndpointAccessGroupWhere(or []string, and []string, in string, span []string, data []byte) *errors.SensError {
	fieldMap := models.GetOpEndpointAccessGroupFieldMap()
	values := make(map[string]interface{})
	...
	for k, _ := range j {
		if f, ok := fieldMap[k]; ok {
			...
			values["set_"+f] = getOpEndpointAccessGroupFieldValue(k, j[k])
			comma = ", "
		}
	}
	//SET ENDS
