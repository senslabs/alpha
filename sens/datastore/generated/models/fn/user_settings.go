...
func BatchInsertUserSetting(data []byte) {
	...
	fmt.Fprint(insert, " ON CONFLICT(user_setting_id) DO UPDATE SET (", strings.Join(fields, ", "), ") = (EXCLUDED.", strings.Join(fields, ", EXCLUDED."), ")")
...
func BatchUpsertUserSetting(data []byte) {
	var j []map[string]interface{}
	types.Unmarshal(data, &j)
	if len(j) == 0 {
		return
...
func UpdateUserSetting(id string, data []byte) {
	...
	fmt.Fprint(update, " WHERE user_setting_id = $", phi)
...
func SelectUserSetting(id string) map[string]interface{} {
	...
	stmt, err := db.Prepare("SELECT * FROM user_settings WHERE user_setting_id = $1")
	defer stmt.Close()
	errors.Pie(err)
...
func DeleteUserSetting(or []string, and []string, in string, span []string) int64 {
	query := bytes.NewBufferString("DELETE FROM user_settings WHERE ")
	var values []interface{}
	buildUserSettingWhereClause(query, or, and, in, span, &values)
	q := query.String()
	logger.Debug(q)
