...
func InsertOpUserAccessGroup(data []byte) (string, error) {
	var j map[string]interface{}
	if err := json.Unmarshal(data, &j); err != nil {
		logger.Error(err)
		return "", errors.FromError(errors.GO_ERROR, err)
	}
	var m models.OpUserAccessGroup
	...
	comma := ""
	fieldMap := models.GetOpUserAccessGroupFieldMap()
	insert := bytes.NewBufferString("INSERT INTO op_user_access_groups(")
...
func BatchInsertOpUserAccessGroup(data []byte) ([]string, error) {
	var j []map[string]interface{}
	if err := json.Unmarshal(data, &j); err != nil {
		logger.Error(err)
	...
	var keys []string
	fieldMap := models.GetOpUserAccessGroupFieldMap()
	insert := bytes.NewBufferString("UPSERT INTO op_user_access_groups(")
...
func buildOpUserAccessGroupWhereClause(query *bytes.Buffer, or []string, and []string, in string, span []string, values map[string]interface{}) {
	...
	for _, o := range ors {
		if f, ok := fieldMap[o.Column]; ok {
			...
			values[f] = getOpUserAccessGroupFieldValue(o.Column, o.Value)
			cond = "OR "
		}
	}
	...
	for _, a := range ands {
		if f, ok := fieldMap[a.Column]; ok {
			...
			values[f] = getOpUserAccessGroupFieldValue(a.Column, a.Value)
		}
	}
	...
	for _, s := range spans {
		if f, ok := fieldMap[s.Column]; ok {
			...
			values["from_"+f] = getOpUserAccessGroupFieldValue(s.Column, s.From)
			values["to_"+f] = getOpUserAccessGroupFieldValue(s.Column, s.To)
		}
	}
	fmt.Fprint(query, "1 = 1)")
}

func getOpUserAccessGroupFieldValue(c string, v interface{}) interface{} {
	typeMap := models.GetOpUserAccessGroupTypeMap()
...
func FindOpUserAccessGroup(or []string, and []string, in string, span []string, limit string, column string, order string) ([]models.OpUserAccessGroup, *errors.SensError) {
	query := bytes.NewBufferString("SELECT * FROM op_user_access_groups WHERE ")
	fieldMap := models.GetOpUserAccessGroupFieldMap()
	values := make(map[string]interface{})
	buildOpUserAccessGroupWhereClause(query, or, and, in, span, values)
	if column != "" {
	...
	if strings.TrimSpace(in) == "" {
		...
		return findOpUserAccessGroupIn(q, values)
	}
}

func UpdateOpUserAccessGroupWhere(or []string, and []string, in string, span []string, data []byte) *errors.SensError {
	fieldMap := models.GetOpUserAccessGroupFieldMap()
	values := make(map[string]interface{})
	update := bytes.NewBufferString("UPDATE op_user_access_groups SET ")
	...
	for k, _ := range j {
		if f, ok := fieldMap[k]; ok {
			...
			values["set_"+f] = getOpUserAccessGroupFieldValue(k, j[k])
			comma = ", "
		}
	}
	//SET ENDS
	...
	buildOpUserAccessGroupWhereClause(update, or, and, in, span, values)
