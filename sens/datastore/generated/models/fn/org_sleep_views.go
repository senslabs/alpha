...
func BatchUpsertOrgSleepView(data []byte) {
	var j []map[string]interface{}
	types.Unmarshal(data, &j)
	if len(j) == 0 {
		return
	...
	fieldMap := models.GetOrgSleepViewFieldMap()
	typeMap := models.GetOrgSleepViewTypeMap()
	insert := bytes.NewBufferString("UPSERT INTO org_sleep_views(")
	for k, _ := range j[0] {
		if f, ok := fieldMap[k]; ok {
			fmt.Fprint(insert, comma, f)
			keys = append(keys, k)
...
func DeleteOrgSleepView(or []string, and []string, in string, span []string) int64 {
	query := bytes.NewBufferString("DELETE FROM org_sleep_views WHERE ")
	var values []interface{}
	buildOrgSleepViewWhereClause(query, or, and, in, span, &values)
	q := query.String()
	logger.Debug(q)
