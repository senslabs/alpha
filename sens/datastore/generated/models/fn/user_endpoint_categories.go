...
func buildUserEndpointCategorieWhereClause(query *bytes.Buffer, or []string, and []string, span []string, values map[string]interface{}) {
	...
	for _, o := range ors {
		if f, ok := fieldMap[o.Column]; ok {
			...
			values[f] = getUserEndpointCategorieFieldValue(o.Column, o.Value)
			cond = "OR "
		}
	}
	...
	for _, a := range ands {
		if f, ok := fieldMap[a.Column]; ok {
			...
			values[f] = getUserEndpointCategorieFieldValue(a.Column, a.Value)
		}
	}
	for _, s := range spans {
		if f, ok := fieldMap[s.Column]; ok {
			fmt.Fprint(query, fmt.Sprintf("%s >= :from_%s AND %s <= :to_%s AND ", f, f, f, f))
			values["from_"+f] = getUserEndpointCategorieFieldValue(s.Column, s.From)
			values["to_"+f] = getUserEndpointCategorieFieldValue(s.Column, s.To)
		}
	}
	fmt.Fprint(query, "1 = 1)")
}

func getUserEndpointCategorieFieldValue(c string, v interface{}) interface{} {
...
func FindUserEndpointCategorie(or []string, and []string, span []string, limit string, column string, order string) ([]models.UserEndpointCategorie, *errors.SensError) {
	...
	buildUserEndpointCategorieWhereClause(query, or, and, span, values)
	if column != "" {
...
func UpdateUserEndpointCategorieWhere(or []string, and []string, span []string, data []byte) *errors.SensError {
	fieldMap := models.GetUserEndpointCategorieFieldMap()
	values := make(map[string]interface{})
	...
	buildUserEndpointCategorieWhereClause(update, or, and, span, values)
