...
func InsertOrgUser(data []byte) (string, error) {
	var j map[string]interface{}
	if err := json.Unmarshal(data, &j); err != nil {
		logger.Error(err)
		return "", errors.FromError(errors.GO_ERROR, err)
	}
	var m models.OrgUser
...
func BatchInsertOrgUser(data []byte) ([]string, error) {
	var j []map[string]interface{}
	if err := json.Unmarshal(data, &j); err != nil {
		logger.Error(err)
...
func buildOrgUserWhereClause(query *bytes.Buffer, or []string, and []string, span []string, values map[string]interface{}) {
	...
	for _, o := range ors {
		if f, ok := fieldMap[o.Column]; ok {
			...
			values[f] = getOrgUserFieldValue(o.Column, o.Value)
			cond = "OR "
		}
	}
	...
	for _, a := range ands {
		if f, ok := fieldMap[a.Column]; ok {
			...
			values[f] = getOrgUserFieldValue(a.Column, a.Value)
		}
	}
	for _, s := range spans {
		if f, ok := fieldMap[s.Column]; ok {
			fmt.Fprint(query, fmt.Sprintf("%s >= :from_%s AND %s <= :to_%s AND ", f, f, f, f))
			values["from_"+f] = getOrgUserFieldValue(s.Column, s.From)
			values["to_"+f] = getOrgUserFieldValue(s.Column, s.To)
		}
	}
	fmt.Fprint(query, "1 = 1)")
}

func getOrgUserFieldValue(c string, v interface{}) interface{} {
	// typeMap := models.GetAuthTypeMap()
	return v
}

func FindOrgUser(or []string, and []string, span []string, limit string, column string, order string) ([]models.OrgUser, *errors.SensError) {
	query := bytes.NewBufferString("SELECT * FROM org_users WHERE ")
	fieldMap := models.GetOrgUserFieldMap()
	values := make(map[string]interface{})
	buildOrgUserWhereClause(query, or, and, span, values)
	if column != "" {
...
func UpdateOrgUserWhere(or []string, and []string, span []string, data []byte) *errors.SensError {
	fieldMap := models.GetOrgUserFieldMap()
	values := make(map[string]interface{})
	update := bytes.NewBufferString("UPDATE org_users SET ")
	...
	buildOrgUserWhereClause(update, or, and, span, values)
