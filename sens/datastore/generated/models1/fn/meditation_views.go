...
func UpdateMeditationView(id string, data []byte) error {
	var j map[string]interface{}
	if err := json.Unmarshal(data, &j); err != nil {
		logger.Error(err)
		return errors.FromError(errors.GO_ERROR, err)
...
func SelectMeditationView(id string) (models.MeditationView, *errors.SensError) {
	db := datastore.GetConnection()
	m := models.MeditationView{}
	if err := db.Get(&m, "SELECT * FROM meditation_views WHERE id = $1", id); err != nil {
		logger.Error(err)
...
func buildMeditationViewWhereClause(query *bytes.Buffer, or []string, and []string, span []string, values map[string]interface{}) {
...
func FindMeditationView(or []string, and []string, span []string, limit string, column string, order string) ([]models.MeditationView, *errors.SensError) {
	query := bytes.NewBufferString("SELECT * FROM meditation_views WHERE ")
	fieldMap := models.GetMeditationViewFieldMap()
	values := make(map[string]interface{})
	buildMeditationViewWhereClause(query, or, and, span, values)
	if column != "" {
...
func UpdateMeditationViewWhere(or []string, and []string, span []string, data []byte) *errors.SensError {
	fieldMap := models.GetMeditationViewFieldMap()
	values := make(map[string]interface{})
	update := bytes.NewBufferString("UPDATE meditation_views SET ")
	...
	buildMeditationViewWhereClause(update, or, and, span, values)
