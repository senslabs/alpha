...
func SelectOrgEndpoint(id string) models.OrgEndpoint {
	db := models.GetConnection()
	m := models.OrgEndpoint{}
	err := db.Get(&m, "SELECT * FROM org_endpoints WHERE id = $1", id)
	if err != nil {
		log.Println(err)
	}
	return m
...
func FindOrgEndpoint(or []string, and []string, span []string) []models.OrgEndpoint {
