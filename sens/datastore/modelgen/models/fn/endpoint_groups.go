...
func SelectEndpointGroup(id string) models.EndpointGroup {
	db := models.GetConnection()
	m := models.EndpointGroup{}
	err := db.Get(&m, "SELECT * FROM endpoint_groups WHERE id = $1", id)
	if err != nil {
		log.Println(err)
	}
	return m
...
func FindEndpointGroup(or []string, and []string, span []string) []models.EndpointGroup {
