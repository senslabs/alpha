...
func SelectOrgUser(id string) models.OrgUser {
	db := models.GetConnection()
	m := models.OrgUser{}
	err := db.Get(&m, "SELECT * FROM org_users WHERE id = $1", id)
	if err != nil {
		log.Println(err)
	}
	return m
...
func FindOrgUser(or []string, and []string, span []string) []models.OrgUser {
