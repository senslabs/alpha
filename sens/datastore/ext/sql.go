...
const (
	...
	TF_LIST_QUERY = `select t.user_id as "UserId", max(t.timestamp) as "Timestamp", t.key as "Key", avg(t.value) as "Value" from (
		...
		select user_id, timestamp as "Timestamp", key as "Key", value as "Value" from session_records where user_id = ANY($1) and key = 'Temperature' and value > 0 order by timestamp desc limit 1
	) union (
		select user_id, timestamp as "Timestamp", key as "Key", value as "Value" from session_records where user_id = ANY($1) and key = 'Spo2' and value > 0 order by timestamp desc limit 1
	) union (
		select user_id, timestamp as "Timestamp", key as "Key", value as "Value" from session_records where user_id = ANY($1) and key = 'PlusStage' and value >= 0 order by timestamp desc limit 1
	)`
